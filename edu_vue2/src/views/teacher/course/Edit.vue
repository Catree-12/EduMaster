...
export default {
  ...
  methods: {
    ...
    // 编辑班期
    editTerm() {
      this.$router.push('/teacher/terms')
    },

    // 编辑班级
    editClass(classItem) {
      this.$router.push(`/teacher/classes?termId=${classItem.termId}`)
    },
    ...
    // 前往班期管理
    goToTermManagement() {
      this.$router.push(`/teacher/terms?courseId=${this.courseId}`)
    },

    // 前往班级管理
    goToClassManagement() {
      this.$router.push(`/teacher/classes?courseId=${this.courseId}`)
    },
