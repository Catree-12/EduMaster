<template>
  <div class="my-courses-container" v-loading="loading">
    ...
    <el-tabs v-model="activeTab" type="card" class="tabs-container">
      <!-- 我教的课 - 仅教师可见 -->
      <el-tab-pane v-if="hasTeacherProfile" label="我教的课" name="teaching">
      ...
      <!-- 我学的课 - 仅学生可见 -->
      <el-tab-pane v-if="hasStudentProfile" label="我学的课" name="learning">
            ...
            <div v-for="course in learningCourses" :key="course.id" class="course-card learning">
              ...
              <div class="course-info">
                ...
                <div class="progress-container">
                  ...
                  <el-progress :percentage="course.progress" :color="progressColor(course.progress)"></el-progress>
                </div>
...
export default {
  ...
  computed: {
    ...
    hasTeacherProfile() {
      return !!this.$store.state.user.userInfo?.teacher_profile
    },
    hasStudentProfile() {
      return !!this.$store.state.user.userInfo?.student_profile
    }
  },
  created() {
    // 加载课程数据
  ...
  methods: {
    ...
    async loadCourses() {
      ...
      try {
        // 调用统一接口获取两种身份下的课程
        const courseAPI = require('@/api/course').courseAPI
        const response = await courseAPI.getMyCourses()
        
        // 后端返回格式: { code: 200, message: '获取成功', data: { student_courses: [], teacher_courses: [] } }
        const data = response.data || response
        const studentCourses = data.student_courses || []
        const teacherCourses = data.teacher_courses || []
        
        // 映射学生课程数据
        this.learningCourses = studentCourses.map(course => ({
          id: course.id,
          title: course.title,
        ...
        // 映射教师课程数据
        this.teachingCourses = teacherCourses.map(course => ({
          id: course.id,
          title: course.title,
          description: course.description || '',
          coverImage: this.getFullMediaUrl(course.cover),
          ...
          termCount: course.term_count || 0,
          createdAt: course.created_at
        }))
        
        // 如果没有任何身份,提示用户
        if (!this.hasTeacherProfile && !this.hasStudentProfile) {
          this.$message.warning('请先完善您的身份信息')
          this.$router.push('/user/profile')
        } else {
          // 默认显示有课程的标签
          if (this.hasTeacherProfile && this.teachingCourses.length > 0) {
            this.activeTab = 'teaching'
          } else if (this.hasStudentProfile && this.learningCourses.length > 0) {
            this.activeTab = 'learning'
          } else if (this.hasTeacherProfile) {
            this.activeTab = 'teaching'
          } else if (this.hasStudentProfile) {
            this.activeTab = 'learning'
          }
        }
      } catch (error) {
    ...
    archiveCourse() {
      this.$confirm('确认下架此课程？', '提示', {
        ...
        .then(async () => {
          try {
            // 注意:后端可能没有下架接口,这里暂时使用删除或更新status的方式
            // 如果后端有专门的archive接口,需要在teacher.js中添加
            this.$message.warning('下架功能待后端接口完善')
            // const teacherAPI = require('@/api/teacher')
            // await teacherAPI.archiveCourse(course.id)
      ...
      this.$confirm('确认退课？退课后将无法继续学习该课程。', '提示', {
        ...
        .then(async () => {
          try {
            const courseAPI = require('@/api/course').courseAPI
            // 使用课程ID退课
            await courseAPI.unenrollCourse(course.id)
