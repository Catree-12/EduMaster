...
1. ✅ **所有路由末尾必须带斜杠 `/`**
2. ⚠️ **后端创建接口返回的ID字段名不同于查询接口**
   - 创建章节返回: `chapter_id`
   - 创建小节返回: `lesson_id`
   - 查询接口返回: `id`
3. ✅ **章节ID和小节ID都是后端生成返回**
4. ✅ **小节的更新/删除不需要传 chapter_id**
5. ✅ **内容块保存是覆盖式的,每次保存会替换该小节的所有内容块**
6. ✅ **文件上传需要先上传到服务器获取URL,再保存到content_blocks**
7. ✅ **前端 document 类型映射为后端 file 类型**
8. ✅ **前端 text 类型映射为后端 rich_text 类型**
9. ✅ **只新建章节不需要保存,点击保存按钮会提示"没有需要保存的内容"**
